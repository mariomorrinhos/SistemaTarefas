...
<head>
    <meta charset="UTF-8">
    <title>Dashboard - Especial de Páscoa</title>
    ...
    <style>
        ...
        :root { 
            --pascoa-grad-1: #a78bfa; /* Roxo Pastel */
            --pascoa-grad-2: #f472b6; /* Rosa Pastel */
            --pascoa-grad-3: #fbbf24; /* Amarelo Ouro */
            --pascoa-grad-4: #34d399; /* Verde Menta */
        }
        
        body { background-color: #fdfbf7; font-family: 'Inter', sans-serif; min-height: 100vh; display: flex; flex-direction: column; overflow-x: hidden; position: relative;}

        /* ========================================================
           ANIMAÇÃO DE FUNDO DO DASHBOARD (OVINHOS FLUTUANTES)
           ======================================================== */
        .easter-bg-container {
            position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
            z-index: -1; overflow: hidden; pointer-events: none;
        }
        .easter-floating-item {
            position: absolute;
            bottom: -10%;
            opacity: 0.15; /* Sutil como marca d'água */
        ...
        /* ========================================================
           TELA DE ANIMAÇÃO INICIAL (SPLASH SCREEN DA PÁSCOA)
           ======================================================== */
        #splash-screen {
        ...
        .easter-egg {
            font-size: 5.5rem;
            animation: bounceHop 1.2s cubic-bezier(0.28, 0.84, 0.42, 1) infinite alternate;
            filter: drop-shadow(0 15px 10px rgba(0,0,0,0.1));
        }

        .easter-egg:nth-child(1) { color: var(--pascoa-grad-1); animation-delay: 0s; }
        .easter-egg:nth-child(2) { color: var(--pascoa-grad-2); animation-delay: 0.2s; }
        .easter-egg:nth-child(3) { color: var(--pascoa-grad-3); animation-delay: 0.4s; }
        .easter-egg:nth-child(4) { color: var(--pascoa-grad-4); animation-delay: 0.6s; }

        @keyframes bounceHop {
            0% { transform: translateY(0) scaleY(0.9) rotate(0deg); }
            100% { transform: translateY(-70px) scaleY(1.05) rotate(15deg); }
        }
        ...
        .navbar-glass { background: rgba(255,255,255,0.9); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(167, 139, 250, 0.2); }
        .navbar-brand { font-weight: 800; color: var(--pascoa-grad-1) !important; letter-spacing: -0.5px; }

        .welcome-section {
            background: linear-gradient(135deg, var(--pascoa-grad-1), var(--pascoa-grad-2)); 
        ...
        .welcome-section::after {
            ...
            position: absolute; right: -10px; bottom: -30px; font-size: 12rem; color: rgba(255,255,255,0.1); transform: rotate(-20deg);
        }
        
        .welcome-text h5 { font-size: 1.6rem; font-weight: 800; margin-bottom: 5px; z-index: 2; position: relative;}
        .welcome-text p { margin: 0; font-size: 0.85rem; opacity: 0.9; z-index: 2; position: relative;}
        ...
        .kpi-card { background: white; border-radius: 20px; padding: 1.2rem; height: 100%; box-shadow: 0 4px 20px rgba(0,0,0,0.03); display: flex; align-items: center; justify-content: space-between; min-height: 100px; transition: transform 0.2s; position: relative; border: 1px solid rgba(167, 139, 250, 0.1); }
        .kpi-card:hover { transform: translateY(-5px); border-color: rgba(167, 139, 250, 0.4); }
        ...
        .kpi-blue { background: #fdf4ff; color: var(--pascoa-grad-1); } 
        .kpi-cyan { background: #fdf2f8; color: var(--pascoa-grad-2); } 
        .kpi-green { background: #f0fdf4; color: var(--pascoa-grad-4); } 
        .kpi-red { background: #fff1f2; color: #e11d48; } 
        .kpi-orange { background: #fffbeb; color: var(--pascoa-grad-3); }
        .kpi-purple { background: #f5f3ff; color: #7c3aed; }
        
        .kpi-calendar-card { display: block !important; padding: 10px !important; }
        .mini-cal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; font-size: 0.75rem; font-weight: 800; color: var(--pascoa-grad-1); text-transform: uppercase; }
        ...
        .mini-cal-today { background: linear-gradient(135deg, var(--pascoa-grad-1), var(--pascoa-grad-2)) !important; color: white !important; }
        ...
        .menu-btn { 
            background: white; border: 1px solid rgba(167, 139, 250, 0.1); border-radius: 16px; padding: 1rem; 
        ...
        .menu-btn:hover { transform: translateY(-4px); box-shadow: 0 10px 25px rgba(167, 139, 250, 0.15); }
        ...
        .chart-header h5 { font-weight: 700; color: #4b5563; font-size: 1rem; margin: 0; }
...
<body>
    ...
    <div id="splash-screen">
        <div class="easter-animation">
            <i class="fa-solid fa-egg easter-egg"></i>
            <i class="fa-solid fa-egg easter-egg"></i>
            <i class="fa-solid fa-egg easter-egg"></i>
            <i class="fa-solid fa-egg easter-egg"></i>
        </div>
        ...
        <div class="container">
            ...
            <div class="welcome-section">
                <div class="welcome-text">
                    <h5>Olá, <?= $primeiroNome ?>. Feliz Páscoa!</h5>
                    <p><i class="fa-solid fa-gift me-1"></i> É tempo de renovação e novas conquistas.</p>
                </div>
    ...
    <script>
        // GERADOR DE FUNDO ANIMADO DA PÁSCOA
        function createEasterBackground() {
            const container = document.createElement('div');
            container.className = 'easter-bg-container';
            document.body.appendChild(container);
            
            const icons = ['fa-egg', 'fa-carrot', 'fa-leaf'];
            const colors = ['#a78bfa', '#f472b6', '#fbbf24', '#34d399']; 
            
            for(let i=0; i<15; i++) {
                const item = document.createElement('i');
                const randomIcon = icons[Math.floor(Math.random() * icons.length)];
                item.className = `fa-solid ${randomIcon} easter-floating-item`;
        ...
        // CONTROLE DA ANIMAÇÃO INICIAL E FUNDO
        ...
        new Chart(document.getElementById('chartStatus'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: ['#34d399', '#a78bfa', '#f472b6'], borderWidth: 0, hoverOffset: 10
                }]
            },
        ...
        // MANTER CORES PADRÃO PARA O FLUXO DE ATIVIDADES
                ...
                datasets: [
                    {
                        ...
                        borderColor: '#fd7e14', // Laranja Padrão
                    ...
                    },
                    {
                        ...
                        borderColor: '#004d26', // Verde Escuro Padrão
                        backgroundColor: (context) => {
                            const ctx = context.chart.ctx;
                            const gradient = ctx.createLinearGradient(0, 0, 0, 350);
            ...
            fetch(`ajax_dashboard_data.php?ano=${anoKpiAtual}`)
                ...
                .then(data => {
                    ...
                    if (data.avisos > 0) {
                        ...
                        cardAvisos.style.borderBottom = '4px solid var(--pascoa-grad-1)';
                    } else {
                        valAvisos.classList.remove('text-danger');
                        labelAvisos.classList.remove('text-danger');
                        cardAvisos.style.borderBottom = '1px solid rgba(167, 139, 250, 0.1)';
                    }
